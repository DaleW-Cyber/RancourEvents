  ...
  const mount=()=>{
    ...
    if(path==='/bingo'){
      ...
      style.textContent=`
        ...
        .player-close{position:sticky;top:0;z-index:5;float:right;border:3px outset #8c6d40;background:#5d181b;color:#efd9aa;padding:5px 9px;cursor:pointer}
        .player-profile-head{display:grid;grid-template-columns:220px minmax(0,1fr);gap:18px;align-items:stretch;margin:2px 0 14px;clear:both}
        .player-character-wrap{height:310px;overflow:hidden;border:3px ridge #957a4c;background:radial-gradient(circle at 50% 38%,#c8b486,#a58c60 75%);box-shadow:inset 0 0 22px rgba(73,48,20,.18)}
        .player-character-frame{display:block;width:100%;height:100%;border:0;background:transparent}
        .player-heading-copy{min-width:0;display:flex;flex-direction:column;justify-content:center}
        .player-kicker{font-size:10px;text-transform:uppercase;letter-spacing:.14em;color:#765729;font-weight:bold}.player-title{margin:4px 0 2px;font-size:34px;color:#591d1a}.player-sub{margin:0 0 14px;color:#654d31;font-size:11px;line-height:1.5}
        .player-profile-note{margin-top:12px;padding:8px 10px;border:2px inset #957a4c;background:#c3ad80;color:#654d31;font-size:10px;line-height:1.45}.player-profile-note a{color:#591d1a;font-weight:bold}
        ...
        @media(max-width:650px){.player-profile-head{grid-template-columns:1fr}.player-character-wrap{height:360px}.player-heading-copy{display:block}.player-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.player-contrib-row{grid-template-columns:1fr auto}.player-contrib-progress{grid-column:1/-1}.player-drop-row{grid-template-columns:48px minmax(0,1fr)}.player-drop-no{grid-column:2}}
      `;
