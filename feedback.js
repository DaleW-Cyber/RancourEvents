...
function readServiceAccount(){if(!SERVICE_ACCOUNT_JSON)return null;try{const value=JSON.parse(SERVICE_ACCOUNT_JSON);if(!value?.client_email||!value?.private_key)return null;return value}catch(error){console.error('Invalid FEEDBACK_GOOGLE_SERVICE_ACCOUNT_JSON:',error.message);return null}}
async function getGoogleAccessToken(){
  if(tokenCache.accessToken&&Date.now()<tokenCache.expiresAt-60000)return tokenCache.accessToken;
  const account=readServiceAccount();if(!account){const error=new Error('Feedback submission storage is not configured.');error.code='FEEDBACK_NOT_CONFIGURED';throw error}
  ...
  const payload=await response.json().catch(()=>({}));if(!response.ok||!payload.access_token)throw new Error(payload.error_description||payload.error||`Google OAuth returned ${response.status}`);tokenCache={accessToken:payload.access_token,expiresAt:Date.now()+Number(payload.expires_in||3600)*1000};return tokenCache.accessToken;
}
...
async function appendFeedback(row){const accessToken=await getGoogleAccessToken(),range=encodeURIComponent(`${FEEDBACK_SHEET_NAME}!A:AD`),url=`https://sheets.googleapis.com/v4/spreadsheets/${FEEDBACK_SHEET_ID}/values/${range}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`;const response=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${accessToken}`,'Content-Type':'application/json'},body:JSON.stringify({values:[row]})});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload?.error?.message||`Google Sheets returned ${response.status}`)}
async function fetchFeedbackRows(){const params=new URLSearchParams({tqx:'out:csv',sheet:FEEDBACK_SHEET_NAME,range:'A:AD'}),url=`https://docs.google.com/spreadsheets/d/${FEEDBACK_SHEET_ID}/gviz/tq?${params}`,response=await fetch(url,{redirect:'follow',headers:{'User-Agent':'RancourEvents/1.0 (+Feedback Results)'}});if(!response.ok)throw new Error(`Google Sheets returned ${response.status}`);const text=await response.text();if(text.trim().startsWith('<!DOCTYPE html')||text.includes('accounts.google.com'))throw new Error('Feedback Sheet is not anonymously readable.');const rows=parse(text,{relax_column_count:true,skip_empty_lines:true}),headers=(rows.shift()||[]).map(value=>String(value||'').trim()),records=rows.filter(row=>row.some(value=>String(value||'').trim())).map(row=>Object.fromEntries(headers.map((header,index)=>[header,String(row[index]??'').trim()])));return{headers,records}}

export function registerFeedbackRoutes(app){
  app.post('/api/feedback',express.json({limit:'120kb'}),async(req,res)=>{try{const feedback=normaliseFeedback(req.body||{}),validationError=validateFeedback(feedback);if(validationError)return res.status(400).json({error:validationError});const responseId=crypto.randomUUID();await appendFeedback(feedbackRow(feedback,responseId));res.set('Cache-Control','no-store');return res.status(201).json({ok:true,responseId})}catch(error){console.error('Feedback submission failed:',error);return res.status(error.code==='FEEDBACK_NOT_CONFIGURED'?503:502).json({error:error.message||'Unable to submit feedback.'})}});
  app.get('/api/feedback-results',async(_req,res)=>{try{const result=await fetchFeedbackRows();res.set('Cache-Control','no-store');return res.json({...result,refreshedAt:new Date().toISOString()})}catch(error){console.error('Feedback results refresh failed:',error);return res.status(503).json({error:error.message||'Unable to load feedback results.'})}});
